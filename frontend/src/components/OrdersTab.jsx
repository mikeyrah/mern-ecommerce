...
import { Check, ChevronDown, ChevronUp, Loader, MapPin, Package, Save, Search, Truck, X } from "lucide-react";
...
const OrdersTab = () => {
  ...
  const reviewReturn = async (order, action) => {
    if (action === "approve" && !window.confirm(`Issue a full $${Number(order.totalAmount).toFixed(2)} refund through Stripe?`)) return;
    setSaving(`return-${order._id}`);
    try {
      const { data } = await axios.post(`/orders/admin/${order._id}/return`, { action, adminNote: order.returnRequest?.adminNote || "" });
      setOrders((current) => current.map((item) => item._id === order._id ? data.order : item));
      toast.success(data.message);
    } catch (error) { toast.error(error.response?.data?.message || "Unable to review return"); }
    finally { setSaving(null); }
  };
    ...
    <div className="mt-5 space-y-4">{visibleOrders.length ? visibleOrders.map((order) => {
      ...
      return <article key={order._id} className="overflow-hidden rounded-[1.5rem] border border-[#e1dacb] bg-white shadow-[0_10px_30px_rgba(68,62,45,0.05)]">
        <button onClick={() => setExpanded(isOpen ? null : order._id)} className="grid w-full gap-4 p-5 text-left sm:grid-cols-[1.2fr_1fr_auto_auto] sm:items-center">
          ...
          <div><p className="text-sm text-[#687064]">{new Date(order.createdAt).toLocaleDateString()}</p><p className="mt-1 text-sm">{order.products.reduce((sum, item) => sum + item.quantity, 0)} items · <span className="capitalize">{order.deliveryMethod || "shipping"}</span></p></div>
          <div><p className="font-serif text-xl font-semibold text-[#b58a34]">${Number(order.totalAmount).toFixed(2)}</p><span className="mt-1 inline-block rounded-full bg-[#e7eee3] px-3 py-1 text-[10px] font-bold uppercase tracking-[0.1em] text-[#536c58]">{order.fulfillmentStatus}</span>{order.returnRequest?.status === "requested" && <span className="ml-2 inline-block rounded-full bg-[#f5ead2] px-3 py-1 text-[10px] font-bold uppercase tracking-[0.1em] text-[#8a6826]">Return requested</span>}</div>
          {isOpen ? <ChevronUp /> : <ChevronDown />}
        </button>
        {isOpen && <div className="border-t border-[#e8e2d7] bg-[#fbfaf6] p-5 sm:p-6">
          <div className="grid gap-6 lg:grid-cols-2">
            <div><h3 className="text-xs font-bold uppercase tracking-[0.16em] text-[#7c9279]">Items</h3><div className="mt-3 space-y-3">{order.products.map((item) => <div key={item._id} className="flex items-center gap-3 rounded-xl bg-white p-3"><span className="flex h-12 w-12 items-center justify-center overflow-hidden rounded-lg bg-[#eef1e8]">{item.image || item.product?.image ? <img src={item.image || item.product?.image} alt="" className="h-full w-full object-cover" /> : <Package size={18} />}</span><div><p className="font-semibold text-[#354139]">{item.name || item.product?.name}</p><p className="text-xs text-[#7c847d]">{item.quantity} × ${Number(item.price).toFixed(2)}</p></div></div>)}</div>{order.shippingAddress?.line1 && <div className="mt-4 flex gap-3 rounded-xl bg-white p-4 text-sm text-[#687064]"><MapPin size={18} className="shrink-0 text-[#7c9279]" /><span>{order.shippingAddress.name}<br />{order.shippingAddress.line1}{order.shippingAddress.line2 ? `, ${order.shippingAddress.line2}` : ""}<br />{order.shippingAddress.city}, {order.shippingAddress.state} {order.shippingAddress.postalCode}</span></div>}{order.deliveryMethod === "pickup" && <div className="mt-4 flex gap-3 rounded-xl bg-[#edf3e9] p-4 text-sm text-[#526757]"><MapPin size={18} className="shrink-0" /><span><strong className="block">Local pickup</strong>{order.pickupLocation || "Pickup details shared after checkout"}</span></div>}</div>
            <div className="space-y-4">{order.returnRequest?.status && order.returnRequest.status !== "none" && <div className="rounded-xl border border-[#dfd6c4] bg-[#fffdf8] p-4"><p className="text-xs font-bold uppercase tracking-[0.14em] text-[#8a6826]">Return {order.returnRequest.status}</p><p className="mt-2 font-semibold text-[#354139]">{order.returnRequest.reason}</p>{order.returnRequest.customerNote && <p className="mt-2 text-sm leading-6 text-[#687064]">{order.returnRequest.customerNote}</p>}{order.returnRequest.status === "requested" && <><label className="mt-4 block text-sm font-semibold text-[#43503f]">Message to customer<textarea value={order.returnRequest.adminNote || ""} onChange={(event) => setOrders((current) => current.map((item) => item._id === order._id ? { ...item, returnRequest: { ...item.returnRequest, adminNote: event.target.value } } : item))} rows="3" className="mt-2 w-full rounded-xl border border-[#dcd5c5] bg-white px-4 py-3 font-normal" placeholder="Return instructions or reason for declining" /></label><div className="mt-3 grid gap-2 sm:grid-cols-2"><button onClick={() => reviewReturn(order, "approve")} disabled={saving === `return-${order._id}`} className="flex items-center justify-center gap-2 rounded-full bg-[#314b3b] px-4 py-2.5 text-sm font-semibold text-white"><Check size={16} /> Approve & refund</button><button onClick={() => reviewReturn(order, "reject")} disabled={saving === `return-${order._id}`} className="flex items-center justify-center gap-2 rounded-full border border-[#b4776e] px-4 py-2.5 text-sm font-semibold text-[#895149]"><X size={16} /> Decline</button></div></>}</div>}<label className="block text-sm font-semibold text-[#43503f]">Fulfillment status<select value={order.fulfillmentStatus} onChange={(event) => updateLocal(order._id, "fulfillmentStatus", event.target.value)} className="mt-2 w-full rounded-xl border border-[#dcd5c5] bg-white px-4 py-3 font-normal capitalize">{statuses.map((status) => <option key={status} value={status}>{status}</option>)}</select></label><div className="grid gap-4 sm:grid-cols-2"><Field label="Carrier" value={order.carrier || ""} onChange={(value) => updateLocal(order._id, "carrier", value)} placeholder="USPS, UPS…" /><Field label="Tracking number" value={order.trackingNumber || ""} onChange={(value) => updateLocal(order._id, "trackingNumber", value)} placeholder="Tracking number" /></div><label className="block text-sm font-semibold text-[#43503f]">Internal note<textarea value={order.adminNote || ""} onChange={(event) => updateLocal(order._id, "adminNote", event.target.value)} rows="3" className="mt-2 w-full rounded-xl border border-[#dcd5c5] bg-white px-4 py-3 font-normal" placeholder="Visible only to administrators" /></label><button onClick={() => saveOrder(order)} disabled={saving === order._id} className="flex w-full items-center justify-center gap-2 rounded-full bg-[#314b3b] px-5 py-3 text-sm font-semibold text-white disabled:opacity-50">{saving === order._id ? <Loader size={17} className="animate-spin" /> : <Save size={17} />} Save order</button>{order.trackingNumber && <p className="flex items-center gap-2 text-xs text-[#687064]"><Truck size={15} /> {order.carrier || "Carrier"}: {order.trackingNumber}</p>}</div>
          </div>
        </div>}
      </article>;
